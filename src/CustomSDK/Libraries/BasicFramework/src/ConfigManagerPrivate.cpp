...
std::vector<spdlog::sink_ptr> ConfigManagerPrivate::getLoggers() const
{
    ...
    if(node_loggers.attribute("enabled").as_bool())
    {
      for(auto& xpath_node_logger : node_loggers.select_nodes("Logger"))
      {
        if(xpath_node_logger.node().attribute("enabled").as_bool())
        {
          ...
          spdlog::sink_ptr logger = nullptr;
          if(type == "ConsoleLogger")
          {
            logger = std::make_shared<spdlog::sinks::stdout_color_sink_mt>(spdlog::color_mode::automatic);
          }
          else if(type == "FileLogger")
          {
            logger = std::make_shared<spdlog::sinks::basic_file_sink_mt>("logs.log",true);
          }
          if(logger)
          {
