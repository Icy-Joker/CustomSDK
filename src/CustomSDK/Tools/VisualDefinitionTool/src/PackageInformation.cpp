...
Q_DECLARE_METATYPE(boost::weak_ptr<EnumerationType>)
Q_DECLARE_METATYPE(boost::weak_ptr<StructureType>)
...
void PackageInformation::showChildDataTypeDefinition()
{
  ...
  if(const boost::shared_ptr<Package> package_shared_ptr = this->package_weak_ptr.lock())
  {
    const std::vector<boost::weak_ptr<EnumerationType>> vector_enumerated_data_type = package_shared_ptr->collectEnumerationType();
    ...
    const std::vector<boost::weak_ptr<StructureType>> vector_structured_data_type = package_shared_ptr->collectStructureType();
