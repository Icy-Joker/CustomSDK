...
{
  qDebug("on_pushButton_Check_clicked");
  ...
  if(std::regex_match(identify_card_number,sub_match,std::regex("^([1-9]\\d{5})(\\d{4})((?:0[1-9])|(?:1[0-2]))((?:[0-2][1-9])|10|20|30|31)(\\d{2})(\\d)([Xx\\d])$")))
  {
    ...
    if(current_parity_bit == valid_parity_bit)
    {
      ...
      const std::string& xxx = sub_match[5];
      ...
      sqlite_database.setDatabaseName("information.db");
      if(sqlite_database.open())
      {
        ...
        QString id_location = QString::fromLocal8Bit("未知地区");
        sqlite_query.exec();
        ...
        QString check_result = QString::fromLocal8Bit("该公民可能为%1-%2-%3在%4某派出所登记的第%5个%6性")
        ...
        ui->textBrowser_Output->setText(QString::fromLocal8Bit("校验成功:\n%1").arg(check_result));
      }
    }
    else
    {
      ui->textBrowser_Output->setText(QString::fromLocal8Bit("校验失败:身份证号无法识别"));
    }
  }
}
