...
bool AbstractFrameworkManager::initializeWithConfig(const char* config_file_path)
{
  bool result = false;
  ...
  if(LogManager::instance()->initializeWithConfig(config_file_path))
  {
    ...
    {
      LOG_INFO(fmt::format("Load Config[{}] Success.",config_file_path).c_str());
    ...
    {
      LOG_ERROR(fmt::format("Load Config[{}] Failed.",config_file_path).c_str());
    }
  }
