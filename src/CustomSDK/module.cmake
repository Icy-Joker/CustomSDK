...
if(${CMAKE_VERSION} VERSION_GREATER_EQUAL 3.13.0)#此模块要求CMake版本至少3.13.0
  ...
  if(${CMAKE_CURRENT_SOURCE_DIR} STREQUAL ${CMAKE_SOURCE_DIR})#编译环境与目标配置初始化
    ...
    if(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")#MSVC编译器
      ...
      if(CMAKE_SIZEOF_VOID_P EQUAL 8)
        ...
        set(PLATFORM "x86" CACHE STRING "PLATFORM")
      endif()
      #库文件调试后缀
      ...
      elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "^(arm)|(arm64)|(aarch64)|(aarch32)$")
        if(CMAKE_SIZEOF_VOID_P EQUAL 8)
          set(PLATFORM "aarch64" CACHE STRING "PLATFORM")
        elseif(CMAKE_SIZEOF_VOID_P EQUAL 4)
          set(PLATFORM "aarch32" CACHE STRING "PLATFORM")
        endif()
      endif()
      #配置运行时库加载路径
      ...
      if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
        ...
        list(APPEND CMAKE_INSTALL_RPATH "$ORIGIN;$ORIGIN/../bin;$ORIGIN/../lib;$ORIGIN/../thirdparty/lib;")
      elseif(CMAKE_CXX_COMPILER_ID STREQUAL "AppleClang")
        list(APPEND CMAKE_INSTALL_RPATH "@executable_path;@loader_path;")
        list(APPEND CMAKE_INSTALL_RPATH ".;../bin;../lib;../thirdparty/lib;")
      endif()
    endif()
  ...
  message(FATAL_ERROR "CURRENT CMAKE_VERSION: (${CMAKE_VERSION}) VERSION_REQUIRED: (3.13.0) OR HIGHER")
endif()
...
macro(configureBoostModules)
  if(DEFINED BOOST_MODULE_LIST)
    if(DEFINED ENV{BOOST_DIR})
      ...
      if(Boost_FOUND)
        if(Boost_VERSION VERSION_GREATER "1.69")
          list(REMOVE_ITEM BOOST_MODULE_LIST "system")#
        endif()
        find_package(Boost OPTIONAL_COMPONENTS ${BOOST_MODULE_LIST} QUIET)
        if(Boost_FOUND)
          if(TARGET "Boost::headers")
            target_link_libraries(${CURRENT_TARGET} PRIVATE "Boost::headers")#
          elseif(TARGET "Boost::boost")
            target_link_libraries(${CURRENT_TARGET} PRIVATE "Boost::boost")#
          endif()
          set(BOOST_MODULE_PREFIX "Boost::")
          list(APPEND BOOST_MODULE_LIST "diagnostic_definitions;disable_autolinking")
...
macro(configureTargetForInstall)
  ...
  if(NOT ${CURRENT_TARGET_TYPE} STREQUAL "UTILITY")#静态库
    if(${CURRENT_TARGET_TYPE} STREQUAL "STATIC_LIBRARY")#静态库
      if(${CMAKE_CURRENT_SOURCE_DIR} MATCHES "${PROJECT_SOURCE_DIR}/01_Libraries")
        if(NOT ${PUBLIC_HEADER_DIR} STREQUAL "${PARENT_DIRECTORY}")
          ...
          install(TARGETS ${CURRENT_TARGET} ARCHIVE DESTINATION "lib/static")#静态库安装目录
        endif()
      endif()
    else()
        ...
        elseif(${CURRENT_TARGET_TYPE} STREQUAL "SHARED_LIBRARY")#动态库
          if(${CMAKE_CURRENT_SOURCE_DIR} MATCHES "${PROJECT_SOURCE_DIR}/01_Libraries")
            if(NOT ${PUBLIC_HEADER_DIR} STREQUAL "${PARENT_DIRECTORY}")#二次开发库(公开)
              install(DIRECTORY "${PUBLIC_HEADER_DIR}/${CURRENT_SOURCE_FOLDER}" DESTINATION "include" OPTIONAL)
              install(TARGETS ${CURRENT_TARGET} RUNTIME DESTINATION "bin" LIBRARY DESTINATION "lib" ARCHIVE DESTINATION "lib")#普通动态库(非插件&&仅运行)安装目录
            else()
              install(TARGETS ${CURRENT_TARGET} RUNTIME DESTINATION "bin" LIBRARY DESTINATION "lib")#普
            endif()
          endif()
        endif()
      endif()
