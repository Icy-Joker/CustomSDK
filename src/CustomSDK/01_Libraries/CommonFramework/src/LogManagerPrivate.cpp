    ...
    if(boost::filesystem::is_regular_file(path) && path.extension().string() == ".xml")
    {
      if(doc.load_file(path.string().c_str()).status == pugi::xml_parse_status::status_ok)
      {
        if(pugi::xml_node node_loggers = doc.select_node("Root/Loggers").node())
        {
          ...
          {
            const spdlog::level::level_enum filter_level = spdlog::level::from_str(node_loggers.attribute("filter_level").as_string("info"));
            const bool async_mode = node_loggers.attribute("async_mode").as_bool(false);
            ...
            for(auto& xpath_node_logger: node_loggers.select_nodes("Logger"))
            {
              if(xpath_node_logger.node().attribute("enabled").as_bool(false))
              {
                ...
                else if(type == "FileLogger")
                {
                  ...
                  const pugi::xml_attribute attribute_max_size = xpath_node_logger.node().attribute("max_size");
                  const pugi::xml_attribute attribute_max_files = xpath_node_logger.node().attribute("max_files");
                  const pugi::xml_attribute attribute_truncate = xpath_node_logger.node().attribute("truncate");

                  try
                  {
                    if(!attribute_max_size.empty() || !attribute_max_files.empty())
                    {
                      const size_t max_size = attribute_max_size.as_ullong(1024 * 1024 * 500);
                      const size_t max_files = attribute_max_files.as_ullong(10);
                      const bool truncate = attribute_truncate.as_bool(true);

                      sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(log_file_path, max_size, max_files, truncate);
                    }
                    else
                    {
                      sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(log_file_path, truncate);
                    }
                  }
                  catch(const std::exception& ex)
                  {
                    std::cerr << fmt::format("Failed to create file sink for '{}': {}", log_file_path, ex.what());
                    sink = nullptr;
                  }
                }
                if(sink)
            ...
            if(async_mode)
            {
              ...
              logger = std::make_shared<spdlog::async_logger>("multi_sink", sinks.begin(), sinks.end(), spdlog::thread_pool(), spdlog::async_overflow_policy::block);
            }
            else
            {
