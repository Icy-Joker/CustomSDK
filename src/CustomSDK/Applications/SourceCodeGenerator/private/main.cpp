  ...
  try
  {
    ...
    if(!map_variables.count("help"))
    {
      if(map_variables.count("project_name"))
      {
        ...
        const std::string project_child_folder_list = map_variables["child_folder"].as<std::string>();
        ...
        {
          std::istringstream iss(project_child_folder_list);
          std::string token;
          while (std::getline(iss, token, ',')) 
          {
            custom_project.appendChildFolder(token);
          }
        }
